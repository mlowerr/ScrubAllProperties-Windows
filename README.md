NOTE: This appears to work, in limited testing performed, but also appears to be an overly complex solution to wrap ExifTool.  (The joys of vibe coding and not testing between steps.)  At some point, I will look at this closer, but right now it solves for the use case I was looking to address, although it took much more time than I wanted to invest into this...
...
- ExifTool is a required dependency when matched files include image/video formats; media metadata removal will fail without it.  (`winget install OliverBetz.ExifTool` to install.)
